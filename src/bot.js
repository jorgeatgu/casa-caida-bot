...
for (let i = 1; i <= datos.length; i++) {
    (function(index) {
        setTimeout(function() {
            let estado = `${datos[i].municipio} en el año 1900 tenía ${
                datos[i].milnovecientos
            } habitantes, en 2018 tiene ${
                datos[i].dosmil
            } habitantes. Ha perdido el #${datos[i].pronvicia}`;
            ...
            bot.post('media/upload', { media_data: b64content }, function (err, data, response) {
              ...
              let altText = `Gráfica sobre la despoblación en ${datos[i].municipio} entre los años 1900 y 2018.`
              let meta_params = { media_id: mediaIdStr, alt_text: { text: altText } }

              bot.post('media/metadata/create', meta_params, function (err, data, response) {
                if (!err) {
                  // now we can reference the media and post a tweet (media will attach to the tweet)
                  let params = { status: estado, media_ids: [mediaIdStr] }
